      ...
      if (entry.mapq >= min_mapq &&
        ...
        if (bc_it != bc_index.end()) {
          ...
          for (auto it = tuples.begin(); it != tuples.end(); ++it) {
            // matches and mismatches consumes query and reference, and
            // contributes to coverage
            if (it->first == SamCigar::Cigar::aln_match ||
                it->first == SamCigar::Cigar::seq_match ||
                it->first == SamCigar::Cigar::seq_mismatch) {

              coverage.add(entry.rname, aln_pos, aln_pos + it->second,
                           FeatureVector<string>(cell_bc));
              aln_pos += it->second;
            }

            // deletions and skipped bases consumes only reference, and
            ...
            // insertions, clipped, and padded bases do not consume reference
            // nothing to be done
